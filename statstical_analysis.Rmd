...
source("data_sources.R", echo = T, prompt.echo = "", spaced = F)
head(arrests_df, 10)
```
...
# update arrest_boro to proper names so that the values that be used in a future merge
...
# get the count of particular offenses per year and per borough
grouped_offenses <- arrests_df %>% 
  group_by(year, arrest_boro, ofns_desc) %>% 
  dplyr::summarize(count = n())
# select the top 5 offenses for each borough
...
From the exploration above, we see that dangerous drugs related offenses are the most prevailent and that this is consistent across boroughs. The next most common offenses are assaults. Lets looks deeper and explore how drug related offenses have changed over time in each borough.
...
```{r}
grouped_arrests <- arrests_df %>% 
  group_by(year, arrest_boro, ofns_desc, age_group, perp_sex, perp_race) %>% 
  dplyr::summarize(count = n())
```
...
source("data_sources.R", echo = F, prompt.echo = "", spaced = F)
head(bronx)
head(queens)
head(brooklyn)
head(manhattan)
...
```{r}
m <- lm(murder_counts ~ arrest_boro + year + perp_race + avg_unemployment_rate, data = merged)
summary(m)
```
```{r}
plot(merged$murder_counts ~ merged$year)
multiLines(m)
```
...
```{r}
m2 <- lm(murder_counts ~ year, data=merged)
#m2 <- lm(murder_counts ~ arrest_boro + year + perp_race + avg_unemployment_rate, data=merged)

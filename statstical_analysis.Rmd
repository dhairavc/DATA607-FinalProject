...
income14_18 <- function (file, table) {
  table_content <- read_xls(path = file, sheet = table, skip = 2) %>%
    na.omit()
  colnames(table_content) = c("boro","year","month","labor_force","employed","unemployed","unemployment_rate")
  final_table <- table_content %>%
    select(boro, year, labor_force, employed, unemployed, unemployment_rate) 
  return(final_table)
}
...
## analysis
```{r}
boxplot(unemployment_rate ~ boro,
        data = income_table,
        ...
        border = "blue", 
        las = 2,
        names = c("Bronx", "Queens", "Brooklyn", "Manhattan", "Staten Island"))
```

...
system.time(arrests_df <- read.socrata("https://data.cityofnewyork.us/resource/8h9b-rp9u.csv?$where=arrest_date >= '2015-01-01T00:00:00.000'&$select=date_extract_y(arrest_date) as year,ofns_desc,arrest_boro,age_group, perp_sex, perp_race",
                     app_token = app_token))
```
...
```{r}
income14_18 <- function (file, table) {
  table_content <- read_xls(path = file, sheet = table, skip = 2) %>%
    na.omit()
  colnames(table_content) = c("area","year","month","labor_force","employed","unemployed","unemployment_rate")
  final_table <- table_content %>%
    select(area, year, labor_force, employed, unemployed, unemployment_rate)
  return(final_table)
}


bronx_income <- income14_18("14-18_income.xls", "Bronx")
queens_income <- income14_18("14-18_income.xls", "Queens")
brooklyn_income <- income14_18("14-18_income.xls", "Brooklyn")
manhattan_income <- income14_18("14-18_income.xls", "Manhattan")
staten_income <- income14_18("14-18_income.xls", "Staten Island")

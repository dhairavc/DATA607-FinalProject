...
by_cat$boro_nm <- as.factor(by_cat$boro_nm)
by_cat$susp_age_group <- as.factor(by_cat$susp_age_group)
by_cat$susp_sex <- as.factor(by_cat$susp_sex)
by_cat$susp_race <- as.factor(by_cat$susp_race)

# m <- lm(crimes ~ boro_nm + susp_age_group + susp_sex + susp_sex, data = by_cat)
# summary(m)
```

```{r}
ggplot(data = by_cat, mapping = aes(x = susp_age_group, y = crimes)) + 
  geom_boxplot()
ggplot(data = by_cat, mapping = aes(x = susp_sex, y = crimes)) + 
  geom_jitter(aes(colour = susp_sex))
ggplot(data = by_cat, mapping = aes(x = susp_race, y = crimes)) + 

...
model <- lm(crimes ~ unemployed, data = joined)
summary(model)
plot(joined$unemployed, joined$crimes, main = "Crimes commited as a function of unemployment",
     xlab = "Unemployment", ylab = "crimes commited",
     pch = 19, frame = FALSE)
abline(model, col = "blue")
```

As shown by the simple linear regression, unemployment has an R-squared  ~0.8 which makes it a better predictor for the crimes number 
...
contrasts(filtered$susp_age_group)
# reference variable is adult, we can rereference by using relevel(var, ref = "new_ref")
```

```{r}
# contrasts(filtered$susp_sex) -> female is the reference
filtered %>%
  mutate(susp_sex = relevel(susp_sex, ref = "M")) -> filtered
# contrasts(filtered$susp_sex)  - > now male is the ref

m <- lm(crimes ~ susp_sex, data = filtered)
summary(m)
```

```{r}
m2 <- lm(crimes ~ susp_age_group + susp_sex + susp_race + boro_nm, data = filtered)
anova(m2)
```

Taking other variables (susp_age_group, susp_race and boro_nm) into account, it can be seen that the categorical variable **susp_race** the most significant variable.

```{r}
summary(m2)
```

It can be seen that being **Black** is significantly associated with an average increase of 1454 in crime compared to other races.

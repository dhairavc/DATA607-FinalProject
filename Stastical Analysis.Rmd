...
ggplot(merged, aes(x=`arrest_boro`, y=unemployment_z, label=unemployment_z)) + 
  ...
  coord_flip()
```
Brox seems to have the largest amount of unemployment rate which is above the average and in the same time it has the highest crime amonhest other counties. 
```{r}
# Data Prep
library(forcats)
merged$boro <- rownames(merged)  # create new column for boro names
...
g + geom_density(aes(fill=factor(age_group)), alpha=0.8) + 
    labs(title="Density plot", 
         ...
         x="Borogh name",
         fill="# cimes committed per age") 
```
We can see that 25-44 age category is the most age category for committing a crime. As demonestrated also, **Bronx** seems to have the highest density of crimes am
```{r}
...
Arrests visualized by location in 2018
...
grouped_offenses <- arrests_df %>% group_by(arrest_boro, ofns_desc) %>% dplyr::summarize(count = n()) %>% arrange(desc(count))
```
```{r}
grouped_boro <- arrests_df %>% group_by(arrest_boro) %>% dplyr::summarize(count = n()) %>% arrange(desc(count))
```
```{r}
